...
for iq=1:7
    keyboard
videoFileReader = vision.VideoFileReader(strcat('G:\STUDY\WhereAmI-master\test\test',int2str(iq),'.mp4'));
...
while ~isDone(videoFileReader)
   ...
   % fprintf('Total %d faces found. Beginning comparison..  \n', size(bbox,1));
    ...
    %f=getframe(Iout);
    %im=frame2im(f);
    imwrite(Iout,strcat(int2str(c), '.jpg'));
